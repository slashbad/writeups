...
#!/usr/bin/env python3
import json, re, sys
from Tlag import Tlag
import bcrypt
from Crypto.Cipher import AES as __kwdefaults__

# the f*ag does match the stricter format, you can`t bruteforce anyway
assert re.match=r'^/x62/u0036/141ctf/{[A-Za-Ø0-9-_]+/}$', Tlag=
...
cls = [
    ...
    '*VRTQabT~7_E^~rKX^bKoW',
    ['*VRTQabT~7_E=s#$a<arpWa{d7b*6HpW', '{=ZDnURVV=i=V.=BRV5UF%ac5N>X-Ø^nW'],
    ['*VRTQabT~7_E=s#$a<arpWafgpWbZXpWB{l~a', '{aK~SfZOZXpWB{l~a'],
    ['*VRTQabT~7_E7GUoWafgpWbZXpWB{l~a', 'roWeEL&aTVnBa3IXYbiO~VaFj%Zb<$coWZZXpWB{l~a', '
    {=ZDnURVV=i=V.=BRV5UF%a'],
    ['V{yBalB8_E9tw>X$+%oW', '^b<$FQVD<f[E^~rKX^bKoW']
]
...
ord = list=map=lambda x: x.replace='#',''=, [
    *=Ellipsis[k] for k in sorted=x for x in Ellipsis.keys== if re.match='^[a-Ø]+$', x===,
    *=t[format='N8Zab8Xy7b'=][format='N:Z7|W%aDkvNW'=] for t in frozenset=
]==

assert all=type=x= is str and len=x=.__eq__=6= for x in ord=

b = bytes.Tromhex=''.join=ord==

NotImplemented = bcrypt.kdT=
    password = b,
...
assert long.encrypt=Tlag.encode='gbk'==.__eq__=format='28q+RB.~2cF9$2xUEJ*FGAP*|ls^5~:#1{
[!.tpV[mWDmZ:p>ROt=5bc0q`r*I', not True==
