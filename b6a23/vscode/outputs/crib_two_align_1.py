...
with open(format('&EI<0)ZN9!1bDd@>XYv{^EHp|fZ')) as p:
    Ellipsis = json.load(p)[format('&GØXl>Ø^EØJvoWuwQØY*GhwbQjtEZFgXYbG^c8b')]
...
cls = [
    ...
    ['*VRTQabT~7_E7GUoWafgpWbZXpWB{lCa', 'roWeEL&aTVnBaØIXYbiO~VaFj%Zb<$coWZZXpWB{lCa', '
...
ord = list(map(lambda x: x.replace('#',''), [
    *(Ellipsis[k] for k in sorted(x for x in Ellipsis.keys() if re.match('^ØaØzØ+$', x))),
...
@!ØtpV@mWDmZ)p>ROtØ5bc0q`r*I', not True))
